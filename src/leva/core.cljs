...
(defui Config [{:keys [opts children]}]
  ...
  (if-let [store (:store opts)]
    ;; if store is defined on opts
    ($ :<>
       ($ l/LevaPanel (assoc opts :store store))
       ($ l/LevaStoreProvider {:store store} children))
    ;; else
    ($ l/Leva opts children)))
...
(defui SubPanel [{:keys [opts children]}]
...
(defui Controls [{:keys [opts]}]
  ...
  (let [[watch-id] (uix.core/use-state (str (random-uuid)))
        !state     (:atom opts)
